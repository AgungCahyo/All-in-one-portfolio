...
/* ─── Leadership cursor — compass rose + vine tail, warm gold/evergreen ─── */
function LeadershipCursor({ x, y, tail, rotation }: { x: number; y: number; tail: TailPoint[]; rotation: number }) {
    ...
    <svg className="pointer-events-none fixed inset-0 w-full h-full overflow-visible" style={{ zIndex: 9999 }}>
      ...
      {tail.map((p, i) => {
        ...
        return (
          <g key={i}>
            <line x1={prev.x} y1={prev.y} x2={p.x} y2={p.y}
              stroke={`rgba(232,162,61,${alpha * 0.6})`}
            ...
            {i % 6 === 0 && progress < 0.75 && (
              <>
                <ellipse cx={(p.x + prev.x) / 2 + nx * 5} cy={(p.y + prev.y) / 2 + ny * 5}
                  ...
                  fill={`rgba(31,110,82,${alpha * 0.4})`} stroke={`rgba(31,110,82,${alpha})`} strokeWidth={0.5}
                />
                {i % 12 === 0 && (
                  <ellipse cx={(p.x + prev.x) / 2 - nx * 5} cy={(p.y + prev.y) / 2 - ny * 5}
                    rx={3} ry={1.5} transform={`rotate(${(angle * 180) / Math.PI}, ${(p.x + prev.x) / 2 - nx * 5}, ${(p.y + prev.y) / 2 - ny * 5})`}
                    fill={`rgba(232,162,61,${alpha * 0.35})`} stroke="none"
                  />
                )}
              </>
            )}
            {i > tail.length - 6 && (
              <circle cx={p.x} cy={p.y} r={2} fill={`rgba(232,162,61,${(1 - progress) * 0.5})`} />
            )}
          </g>
        );
      })}
      ...
      {/* Outer ring */}
      <circle cx={x} cy={y} r={30} stroke="rgba(31,110,82,0.28)" strokeWidth={0.5} fill="none" />
        ...
        return (
          <g key={i}>
            <line x1={x} y1={y} x2={tx} y2={ty}
              stroke={`rgba(232,162,61,${isPrimary ? 0.4 : 0.2})`}
              strokeWidth={isPrimary ? 0.8 : 0.5}
            />
            {isPrimary && (
              <rect
                x={tx - 2} y={ty - 2} width={4} height={4}
                fill="rgba(31,110,82,0.6)"
      ...
      {fronds.map(i => {
        ...
        return (
          <path key={i}
            ...
            stroke="rgba(232,162,61,0.25)" strokeWidth={0.8} fill="none"
          />
        );
      })}

      {/* Inner ring */}
      <circle cx={x} cy={y} r={10} stroke="rgba(31,110,82,0.65)" strokeWidth={1} fill="rgba(232,162,61,0.1)" />

      {/* Center drop */}
      <circle cx={x} cy={y} r={2.5} fill="rgba(31,110,82,0.9)" />
    </svg>
  );
}
...
export function CursorFollower() {
  ...
  return (
    <>
      ...
      {role === 'leadership' && <LeadershipCursor x={x} y={y} tail={tail} rotation={rotation} />}
    </>
  );
}